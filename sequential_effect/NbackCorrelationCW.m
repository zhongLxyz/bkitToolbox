%function meanResults = NbackCorrelationCW(allData,N,varargin)
...
allData = bkitRead('CCW');
variable = 'vfa';  % what variable you want to look at ('ccw','ftv','vfa')
N = 6;  % N-back response;

% results is a 3D array, in each layer it stores the 1 to N-back
% corelation results of one session.
...
for i=1:length(allData)
    ...
    if strcmp('ftv',variable)
        d = findFTV(angularVel,azimuth,response,elevation,'ccw');  % d is the facing direction perceived. 1: FTV, 0: FA
        for j = 1:N
            [RHO,PVAL] = corr(d(1:end-j),d(j+1:end));
            previousTrial(:,j) = [RHO; PVAL]; % row 1: r-value, row 2: p-valuel; each column is N
        end
    % finding perceived turning direction
    elseif strcmp('ccw',variable)
        d = findCCW(angularVel,azimuth,response,elevation,'ccw');
        for j = 1:N
            [RHO,PVAL] = corr(d(1:end-j),d(j+1:end));
            previousTrial(:,j) = [RHO; PVAL];
        end
    % finding perceived viewing perspective
    elseif strcmp('vfa',variable)
        d = findVFA(angularVel,azimuth,response,elevation,'ccw');
        for j = 1:N
            [RHO,PVAL] = corr(d(1:end-j),d(j+1:end));
            previousTrial(:,j) = [RHO; PVAL];
    ...
    results(:,:,i) = previousTrial; % each layer (the 3rd dimension) is for one session
