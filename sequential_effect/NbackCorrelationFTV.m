%function meanResults = NbackCorrelationFTV(allData,N,varargin)
...
allData = bkitRead('FTV');
variable = 'ftv';  % what variable you want to look at ('ccw','ftv','vfa')
N = 6;  % N-back response;
...
for i=1:length(allData)
    ...
    if strcmp('ftv',variable)
        d = findFTV(angularVel,azimuth,response,elevation,'ftv');  % d is the facing direction perceived. 1: FTV, 0: FA
        for j = 1:N
            [RHO,PVAL] = corr(d(1:end-j),d(j+1:end));
            previousTrial(:,j) = [RHO; PVAL]; 
        end
    elseif strcmp('ccw',variable)
        d = findCCW(angularVel,azimuth,response,elevation,'ftv');
        for j = 1:N
            [RHO,PVAL] = corr(d(1:end-j),d(j+1:end));
            previousTrial(:,j) = [RHO; PVAL]; 
        end
    elseif strcmp('vfa',variable)
        d = findVFA(angularVel,azimuth,response,elevation,'ftv');
        for j = 1:N
            [RHO,PVAL] = corr(d(1:end-j),d(j+1:end));
            previousTrial(:,j) = [RHO; PVAL];
